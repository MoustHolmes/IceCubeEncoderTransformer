...
app.layout = html.Div([
    dcc.Dropdown(
        id='event-dropdown',
        options=[{'label': event, 'value': event} for event in available_event_numbers],
        value=available_event_numbers[0],  # Default value
        clearable=False
    ),
    dcc.Graph(id='event-plot', figure=fig, style={'width': '100%', 'height': '100%'}),
    dcc.Store(id='stored-layout'),  # Display the created figure
], style={'width': '100vw', 'height': '100vh', 'margin': '0px'})

@app.callback(
    Output('stored-layout', 'data'),
    [Input('event-plot', 'relayoutData')],
...
def update_figure(selected_event, stored_layout):
    # Load the data for the selected event
    truth = load_Truth_from_sql(selected_event, db_path)
    attn_weights = load_attention_weights(selected_event, attn_weights_db_path)
    data_clean = load_data_clean(selected_event, db_path, attn_weights)
    

    data_clean.sort_values(by=['dom_time'], inplace=True)

    # Update interaction vertex data
    fig.data[4].x = truth['position_x']
    fig.data[4].y = truth['position_y']
    fig.data[4].z = truth['position_z']

    # Update direction vector data
    fig.data[5].x = [truth['position_x'].iloc[0], truth['position_x'].iloc[0] + r*np.cos(truth['azimuth'].iloc[0])*np.sin(truth['zenith'].iloc[0])]
    fig.data[5].y = [truth['position_y'].iloc[0], truth['position_y'].iloc[0] + r*np.sin(truth['azimuth'].iloc[0])*np.sin(truth['zenith'].iloc[0])]
    fig.data[5].z = [truth['position_z'].iloc[0], truth['position_z'].iloc[0] + r*np.cos(truth['zenith'].iloc[0])]

    # Initialize DOM hit data as empty
    fig.data[2].x = []
    fig.data[2].y = []
    fig.data[2].z = []
    ...
    if stored_layout and 'scene.camera' in stored_layout:
        fig['layout']['scene']['camera'] = stored_layout['scene.camera']
