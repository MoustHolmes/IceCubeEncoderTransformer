...
class LocalBlock(nn.Module):
    ...
    def forward(self, x, nbs, key_padding_mask=None, rel_pos_bias=None):
        ...
        mask = (
            ...
            else torch.ones(B, Lmax, dtype=torch.bool, device=self.device)
        )

        m = torch.gather(mask.unsqueeze(1).expand(-1, Lmax, -1), 2, nbs)
        attn_mask = torch.zeros(m.shape, device=self.device)
        ...
        x = torch.zeros(x.shape, device=self.device, dtype=xl.dtype)
        x[mask] = xl.squeeze(1)
        return x

class SinusoidalPosEmb(nn.Module):
    def __init__(self,device, dim=16, M=10000):
        super().__init__()
        self.device = device
        self.dim = dim
        self.M = M
...
class Extractor(nn.Module):
    def __init__(self, device, dim_base=128, dim=384):
        super().__init__()
        self.device = device
...
class DeepIceModel(nn.Module):
    ...
    def forward(self, x0):
        ...
        mask = torch.cat(
            [torch.ones(B, 1, dtype=mask.dtype, device=self.device), mask], 1
        )
        attn_mask = torch.zeros(mask.shape, device=self.device)
        attn_mask[~mask] = -torch.inf
